...
function ChatContainer() {
  ...
  return (
    <section className="ui-chat-container flex-1 flex flex-col">
        ...
        <div className="group relative">
          ...
          <div className="p-2 shadow-xl rounded absolute top-full bg-slate-600 right-0 hidden group-hover:block">
            {selectedRoom?.users?.map((user) => (
              <div className="p-1 border-b">{user}</div>
            ))}
          </div>
        </div>
      </header>
      ...
      <footer className="bg-slate-800 p-3 flex gap-2">
        <input
          ...
          className="border rounded flex-1 py-1 pl-8 px-2 focus:outline-none"
        />
        <Button
          type="button"
          className="w-24 hover:border"
