...
export function RoomProvider({ children }) {
  ...
  const getRoomsList = async () => {
    try {
      ...
      const alreadySelectedRoom = roomsList.find(
        (room) => room.id === selectedRoom?.id
      );
      if (alreadySelectedRoom) {
        const roomWithOwnerProps = addOwnerPropsToRoom(
          alreadySelectedRoom,
          user
        );
  ...
  const joinRoom = async (room) => {
    ...
    try {
      await updateDoc(doc(firebaseDB, "rooms", room.id), {
        users: [...room.users, user.email],
      });
      getRoomsList();
    } catch (error) {
